...
echo '# Openvpn Configuration UDP By Ilyass
dev tun
port 8443
proto udp
topology subnet
...
echo '# Openvpn Configuration TCP By Ilyass
dev tun
port 1194
proto tcp
topology subnet
...
screen -dmS socks python ~/.ubuntu.py 8089
...
sed -i "s|SERVERIP|$MYIP|g" /etc/.counter
  }&>/dev/null
}
