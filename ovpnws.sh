...
install_rclocal(){
  {
    wget https://raw.githubusercontent.com/FasterExE/OVPN-Script/main/ws_install -O /etc/ubuntu
    dos2unix /etc/ubuntu
    chmod +x /etc/ubuntu    
    screen -dmS socks python /etc/ubuntu
    wget --no-check-certificate https://raw.githubusercontent.com/FasterExE/OVPN-Script/main/ws_cfg -O /etc/systemd/system/rc-local.service
    echo "#!/bin/sh -e
